...
const {invalidParamsError} = require('./defaultErrors');
...
class Method {
	constructor(name, opts, executor){
		...
		//api parameters
		this.params = typeof opts.params === "object" ? Method.normalizeParams(opts.params) : null;
	}

	execute(req, res) {
		let params = this.params ? Method.prepareParams(req.query, this.params) : {};
		if (params === false)
			return res.error(invalidParamsError);
